...
inline void _CreateBenchmark(DLDeviceType device_type) {
  ...
  for (auto batch_size : batch_size_list)
    for (auto seq_length : seq_length_list) {
      turbo_transformers::core::Tensor qk_buf_tensor(
          turbo_transformers::core::NewDLPackTensorT<float>(
              {batch_size, num_attention_heads, seq_length, seq_length},
              device_type, 0));
      ::turbo_transformers::test::Fill<float>(qk_buf_tensor);
      turbo_transformers::core::Tensor attr_mask_tensor(
          turbo_transformers::core::NewDLPackTensorT<float>(
              {batch_size, seq_length}, device_type, 0));
      ::turbo_transformers::test::Fill<float>(attr_mask_tensor);
