...
<<<<<<< HEAD:easy_transformers/layers/kernels/activation.cpp
...
>>>>>>> 4ae376e7b39eef1ed828eb6035d1955ea565369c:turbo_transformers/layers/kernels/activation.cpp
    #ifdef FT_WITH_CUDA
...
#pragma omp simd
      for (int64_t j = feature_dim * i; j < feature_dim * (i + 1); ++j) {
        ...
        buff[j] =
            (0.7978845608028654f * (tmp_ + 0.044715f * tmp_ * tmp_ * tmp_));
      }
...
#pragma omp simd
  ...
  template <typename T>
  void AddBiasGeLUAct(const core::Tensor& bias_tensor,
  ...
  void AddBiasGeLUAct<core::Half>(const core::Tensor& bias_tensor,
                                  core::Tensor* out_tensor) {
    FT_ENFORCE_EQ(bias_tensor.device_type(), kDLGPU,
                  "The device should be GPU.");
