...
void PrepareBertMasks::operator()(const core::Tensor& inputs,
  ...
  if (position_ids->is_null()) {
    ...
    for (int64_t row_id = 0; row_id < inputs.shape(0); ++row_id) {
      kernels::common::tt_seqence(pos_ids_ptr, inputs.shape(1),
                                  inputs.device_type());
  ...
  if (seq_type->is_null()) {
    ...
    kernels::common::tt_fill(seq_type->mutableData<int64_t>(),
  ...
  if (att_mask->is_null()) {
    att_mask->Reshape<int64_t>({inputs.shape(0), inputs.shape(1)},
                               inputs.device_type(), inputs.device_id());
    kernels::common::tt_fill(att_mask->mutableData<int64_t>(),
  ...
  extended_attention_mask->Reshape<float>(
      ...
      inputs.device_id());
  kernels::common::tt_transform(att_mask->mutableData<int64_t>(),
