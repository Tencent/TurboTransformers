...
void LayerNorm(const core::Tensor& gamma, const core::Tensor& beta,
  ...
  FT_ENFORCE_EQ(core::is_same_device_ctx(gamma.device_ctx(), beta.device_ctx()),
                true,
...
void AddBiasLayerNorm(const core::Tensor& input_tensor,
                      ...
                      core::Tensor* out_tensor) {
  FT_ENFORCE_EQ(
      core::is_same_device_ctx(input_tensor.device_ctx(),
                               bias_tensor.device_ctx()) &&
          core::is_same_device_ctx(gamma_tensor.device_ctx(),
                                   beta_tensor.device_ctx()) &&
          core::is_same_device_ctx(input_tensor.device_ctx(),
                                   gamma_tensor.device_ctx()),
      true,
      "AddBiasLayerNorm input_tensor, bias_tensor, gamma_tensor and beta_tensor"
      "must be on the same device context.");
