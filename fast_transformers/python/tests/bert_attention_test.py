...
def create_test(batch_size, seq_length):
    class TestBertAttention(unittest.TestCase):
        def setUp(self) -> None:
            ...
            self.ft_self_attention = fast_transformers.BertSelfAttention(_((qkv_weight)),
                                                                _(qkv_bias),
            ...
            self.ft_attention = fast_transformers.BertAttention(_((attention_params['self.query.weight'])),
                                                                _(attention_params['self.query.bias']),
                                                                _((attention_params['self.key.weight'])),
                                                                _(attention_params['self.key.bias']),
                                                                _((attention_params['self.value.weight'])),
                                                                _(attention_params['self.value.bias']),
                                                                _((attention_params['output.dense.weight'])),
                                                                _(attention_params['output.dense.bias']),
                                                                _(attention_params['output.LayerNorm.weight']),
                                                                _(attention_params['output.LayerNorm.bias']),
        ...
        def test_bertattention(self):
            ...
            print(f"BertAttention({batch_size}, {seq_length}) Torch QPS, {self.num_iter / t.elapsed}, Elapse {t.elapsed / self.num_iter}")
            ...
            print(f"BertAttention({batch_size}, {seq_length}) JIT QPS, {self.num_iter / t.elapsed}, Elapse {t.elapsed / self.num_iter}")
            ...
            with contexttimer.Timer() as t:
                for it in range(self.num_iter):
                    self.onnxruntime_attention.run(inputs=onnx_inputs)
            print(f'BertAttention({batch_size}, {seq_length}) ONNX(MKL-DNN) QPS, {self.num_iter / t.elapsed}, Elapse {t.elapsed / self.num_iter}')
            ...
            print(f"BertAttention({batch_size}, {seq_length}) FastTransform QPS, {self.num_iter / t.elapsed}, Elapse {t.elapsed / self.num_iter}")
            # print("max diff: ", torch.max(torch.abs(torch_attention_result[0] - ft_attention_result)))
            ...
            ft_self_attention_result = dlpack.from_dlpack(
                self.ft_self_attention(_(self.input_tensor), _(self.attention_mask), _(self.head_mask)).to_dlpack());
            with contexttimer.Timer() as t:
                for it in range(self.num_iter):
                    ft_self_attention_result = dlpack.from_dlpack(
                        self.ft_self_attention(_(self.input_tensor), _(self.attention_mask), _(self.head_mask)).to_dlpack());

            print(f"BertAttention({batch_size}, {seq_length}) BertSelfAttention QPS, {self.num_iter / t.elapsed}, Elapse {t.elapsed / self.num_iter}")
