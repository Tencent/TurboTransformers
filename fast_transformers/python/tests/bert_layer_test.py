...
class FastBertLayer(nn.Module):
    def __init__(self, config, bert_layer_params, isSelf: bool):
        ...
        qkv_weight = torch.cat(
            (bert_layer_params['attention.self.query.weight'],
             bert_layer_params['attention.self.key.weight']), 0)
        qkv_weight = torch.cat(
            (qkv_weight, bert_layer_params['attention.self.value.weight']), 0)

        qkv_bias = torch.cat((bert_layer_params['attention.self.query.bias'],
                              bert_layer_params['attention.self.key.bias']), 0)
        qkv_bias = torch.cat(
            (qkv_bias, bert_layer_params['attention.self.value.bias']), 0)

        self.attention = fast_transformers.BertAttention(
            convert2ft_tensor(qkv_weight), convert2ft_tensor(qkv_bias),
            convert2ft_tensor(
                (bert_layer_params['attention.output.dense.weight'])),
            convert2ft_tensor(
                bert_layer_params['attention.output.dense.bias']),
            convert2ft_tensor(
                bert_layer_params['attention.output.LayerNorm.weight']),
            convert2ft_tensor(
                bert_layer_params['attention.output.LayerNorm.bias']),
            config.num_attention_heads)

        self.intermediate = fast_transformers.BertIntermediate(
            convert2ft_tensor(bert_layer_params['intermediate.dense.weight']),
            convert2ft_tensor(bert_layer_params['intermediate.dense.bias']))

        self.output = fast_transformers.BertOutput(
            convert2ft_tensor((bert_layer_params["output.dense.weight"]
                               )),  # trans here Important
            convert2ft_tensor(bert_layer_params["output.dense.bias"]
                              ),  # bertout_params["dense.bias"],
    ...
    def forward(self,
        ...
        self_attention_outputs = self.attention(
            convert2ft_tensor(hidden_states),
            convert2ft_tensor(attention_mask), convert2ft_tensor(head_mask))
...
class TestBertLayer(unittest.TestCase):
    def setUp(self) -> None:
        ...
        bert_layer_params = {
            k: v
            for k, v in self.torch_bert_layer.named_parameters()
        }
    ...
    def test_bert_layer(self):
        ...
        ft_bert_layer_result = dlpack.from_dlpack(
            ft_bert_layer_result.to_dlpack())
