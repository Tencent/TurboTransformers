...
inline void _CreateBenchmark(DLDeviceType device_type) {
  ...
  for (auto batch_size : batch_size_list)
    for (auto seq_length : seq_length_list) {
      easy_transformers::core::Tensor qk_buf_tensor(
          easy_transformers::core::NewDLPackTensorT<float>(
              {batch_size, num_attention_heads, seq_length, seq_length},
              device_type, 0));
      ::easy_transformers::test::Fill<float>(qk_buf_tensor);
      easy_transformers::core::Tensor attr_mask_tensor(
          easy_transformers::core::NewDLPackTensorT<float>(
              {batch_size, seq_length}, device_type, 0));
      ::easy_transformers::test::Fill<float>(attr_mask_tensor);
...
TEST_CASE("softmax CPU and GPU correctness") {
  ...
  for (auto batch_size : batch_size_list)
    for (auto seq_length : seq_length_list) {
      easy_transformers::core::Tensor qk_buf_gpu(
          easy_transformers::core::NewDLPackTensorT<float>(
              {batch_size, num_attention_heads, seq_length, seq_length}, kDLGPU,
              0));

      easy_transformers::core::Tensor qk_buf_cpu(
          easy_transformers::core::NewDLPackTensorT<float>(
              {batch_size, num_attention_heads, seq_length, seq_length}, kDLCPU,
              0));
      ::easy_transformers::test::FillDataForCPUGPUTensors<float>(qk_buf_cpu,
                                                                 qk_buf_gpu);

      easy_transformers::core::Tensor attr_mask_gpu(
          easy_transformers::core::NewDLPackTensorT<float>(
              {batch_size, seq_length}, kDLGPU, 0));

      easy_transformers::core::Tensor attr_mask_cpu(
          easy_transformers::core::NewDLPackTensorT<float>(
              {batch_size, seq_length}, kDLCPU, 0));

      ::easy_transformers::test::FillDataForCPUGPUTensors<float>(attr_mask_cpu,
                                                                 attr_mask_gpu);
      ...
      REQUIRE(::easy_transformers::test::CompareCPUGPU<float>(qk_buf_cpu,
                                                              qk_buf_gpu));
    }
}
