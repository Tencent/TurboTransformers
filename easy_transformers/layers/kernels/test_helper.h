...
using Tensor = easy_transformers::core::Tensor;
...
void FillDataForCPUGPUTensors(Tensor& cpu_tensor, Tensor& gpu_tensor) {
  ...
  easy_transformers::core::FT_Memcpy(
      gpu_data, cpu_data, size * sizeof(T),
      ::easy_transformers::core::MemcpyFlag::kCPU2GPU);
}
...
bool CompareCPUGPU(const Tensor& cpu_tensor, const Tensor& gpu_tensor) {
  ...
  easy_transformers::core::FT_Memcpy(
      ...
      easy_transformers::core::MemcpyFlag::kGPU2CPU);
  bool ret = true;
  for (int64_t i = 0; i < size; ++i) {
...
template <typename T>
void Fill(easy_transformers::core::Tensor& tensor, T lower_bound = 0.,
          T upper_bound = 1.) {
  ...
  easy_transformers::core::MemcpyFlag flag;
  if (tensor.device_type() == kDLCPU) {
    flag = easy_transformers::core::MemcpyFlag::kCPU2CPU;
  } else if (tensor.device_type() == kDLGPU) {
    flag = easy_transformers::core::MemcpyFlag::kCPU2GPU;
  } else {
    FT_THROW("Fill device_type wrong");
  }
  ::easy_transformers::core::FT_Memcpy(T_data, cpu_data.get(), size * sizeof(T),
                                       flag);
}
