...
TEST_CASE("activation CPU benchmark") {
  ...
  for (auto batch_size : batch_size_list)
    for (auto seq_length : seq_length_list) {
      ...
      easy_transformers::core::Tensor bias(nullptr);
      bias.Reshape<float>({n}, kDLCPU, 0);
      easy_transformers::core::Tensor out(nullptr);
      out.Reshape<float>({m, n}, kDLCPU, 0);
      easy_transformers::core::Tensor out_parallel(nullptr);
...
TEST_CASE("activation GPU benchmark") {
  ...
  for (auto batch_size : batch_size_list)
    for (auto seq_length : seq_length_list) {
      ...
      easy_transformers::core::Tensor bias(
          easy_transformers::core::NewDLPackTensorT<float>({hidden_size},
                                                           kDLGPU, 0));
      ::easy_transformers::test::Fill<float>(bias);

      easy_transformers::core::Tensor out(
          easy_transformers::core::NewDLPackTensorT<float>(
              {batch_size, seq_length, hidden_size}, kDLGPU, 0));
