...
2. Tailored to NLP inference tasks. Unlike the CV task, the input dimensions of the NLP inference task always change. The traditional approach is zero padding or truncation to a fixed length, which introduces additional zero padding computational overhead. Besides, some frameworks such as onnxruntime, tensorRT, and torchlib need to preprocess the calculation graph according to the input size in advance, which is not suitable for NLP tasks with varying sizes. Although some mechanisms are designed to alleviate this problem (like dynamic_axis trick for onnxruntime), we still did not found a very good solution for transformer models. TurboTransformers can support variable-length input sequence processing without preprocessing.
